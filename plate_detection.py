...
# LOCALITZACIÓ DE MATRÍCULA DEL VEHICLE
...
def detect_plate(img):
    ...
    blackhat = cv2.morphologyEx(gray, cv2.MORPH_BLACKHAT, rectKern) #ressalta contrast negre-blanc

    _, light = cv2.threshold(blackhat, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU) #extreu regions clares (thr = 255)

    gradX = cv2.Sobel(blackhat, cv2.CV_32F, 1, 0, ksize=-1) #deteccio de contorns
    gradX = cv2.convertScaleAbs(gradX)
    gradX = cv2.GaussianBlur(gradX, (5,5), 0)
    gradX = cv2.morphologyEx(gradX, cv2.MORPH_CLOSE, rectKern)
    _, thresh = cv2.threshold(gradX, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)

    thresh = cv2.erode(thresh, None, iterations=2) #netejar mask
